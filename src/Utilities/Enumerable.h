...
public:
	...
	static void LoadFromINIList(CCINIClass* pINI)
	{
		...
		for (int i = 0; i < len; ++i)
		{
			if (pINI->ReadString(section, pINI->GetKeyName(section, i), "", YRAggressiveStance::readBuffer))
				FindOrAllocate(YRAggressiveStance::readBuffer);
		}
	...
	static bool LoadGlobals(YRAggressiveStanceStreamReader& Stm)
	{
		...
		for (size_t i = 0; i < Count; ++i)
		{
			...
			YRAggressiveStanceSwizzle::RegisterChange(oldPtr, newPtr);
	...
	void LoadFromStream(YRAggressiveStanceStreamReader& Stm) = delete;

	void SaveToStream(YRAggressiveStanceStreamWriter& Stm) = delete;
};

...
public:
	...
	virtual inline void SaveToStream(YRAggressiveStanceStreamWriter& Stm)
	{
		//Stm.Save(this->AttachedToObject);
		Stm.Save(this->Initialized);
	}

	virtual inline void LoadFromStream(YRAggressiveStanceStreamReader& Stm)
	{
...
protected:
	...
	extension_type_ptr SaveKey(base_type_ptr key, IStream* pStm)
	{
		...
		YRAggressiveStanceByteStream saver(sizeof(*buffer));
		YRAggressiveStanceStreamWriter writer(saver);
	...
	extension_type_ptr LoadKey(base_type_ptr key, IStream* pStm)
	{
		...
		YRAggressiveStanceByteStream loader(0);
		...
		YRAggressiveStanceStreamReader reader(loader);
